...
#include "actions/ferm/fermacts/eoprec_expo_clover_fermact_w.h"
...
namespace Chroma
{
  ...
  namespace EvenOddPrecExpoCloverFermActEnv
  {
    ...
    {
      return new EvenOddPrecExpoCloverFermAct(CreateFermStateEnv::reader(xml_in, path), 
    ...
    //! Name to be used
    const std::string name = "EXPO_CLOVER";
  ...
  EvenOddPrecLogDetLinearOperator<LatticeFermion,
				  multi1d<LatticeColorMatrix>,
				  multi1d<LatticeColorMatrix> >* 
  EvenOddPrecExpoCloverFermAct::linOp(Handle< FermState<T,P,Q> > state) const
  {
    return new EvenOddPrecExpoCloverLinOp(state,param);
  }
