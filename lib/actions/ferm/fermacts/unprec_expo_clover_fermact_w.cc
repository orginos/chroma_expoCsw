...
#include "actions/ferm/linop/unprec_expo_clover_linop_w.h"
#include "actions/ferm/fermacts/unprec_expo_clover_fermact_w.h"
...
namespace Chroma
{
  ...
  namespace UnprecExpoCloverFermActEnv
  {
    ...
    {
      return new UnprecExpoCloverFermAct(CreateFermStateEnv::reader(xml_in, path), 
    ...
    //! Name to be used
    const std::string name = "UNPRECONDITIONED_EXPO_CLOVER";
  ...
  UnprecLinearOperator<LatticeFermion,
		       multi1d<LatticeColorMatrix>,
		       multi1d<LatticeColorMatrix> >* 
  UnprecExpoCloverFermAct::linOp(Handle< FermState<T,P,Q> > state) const
  {
    return new UnprecExpoCloverLinOp(state,param);
  }
