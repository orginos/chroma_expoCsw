...
#ifndef __prec_expo_clover_linop_w_h__
#define __prec_expo_clover_linop_w_h__
...
#include "actions/ferm/linop/expo_clover_term_w.h"


namespace Chroma 
{ 
  //! Even-odd preconditioned Exponentiated Clover-Dirac operator
  /*!
   * \ingroup linop
   *
   ...
   *      M  =  A + (d+M) - (1/2) D'
   */
  class EvenOddPrecExpoCloverLinOp : public EvenOddPrecLogDetLinearOperator<LatticeFermion, 
  ...
  public:
    ...
    //! Partial constructor
    EvenOddPrecExpoCloverLinOp() {}

    //! Full constructor
    EvenOddPrecExpoCloverLinOp(Handle< FermState<T,P,Q> > fs,
    ...
    ~EvenOddPrecExpoCloverLinOp() {
      QDPIO::cout << "CLOV_LINOP: Time spent in expo clov deriv (total) = " << clov_deriv_time << std::endl;
      QDPIO::cout << "CLOV_LINOP: Time spent in expo clov apply/invapply (total) = " << clov_apply_time << std::endl;
  ...
  private:
    ...
    WilsonDslash D;
    ExpoCloverTerm   clov;
    ExpoCloverTerm   invclov;  // uggh, only needed for evenEvenLinOp
