// -*- C++ -*-
/*! \file
 *  \brief Exponentiated Clover term linear operator
 */

#ifndef __expo_clover_term_qdp_w_h__
#define __expo_clover_term_qdp_w_h__
...
{ 
  ...
  class QDPExpoCloverTermT : public CloverTermBase<T, U>
  {
  public:
    ...
    virtual void create(Handle< FermState<T, multi1d<U>, multi1d<U> > > fs,
			const CloverFermActParams& param_,
			const QDPExpoCloverTermT<T,U>& from_);
  ...
  private:
   ...
   // Empty constructor. Must use create later
  template<typename T, typename U>
  QDPExpoCloverTermT<T,U>::QDPExpoCloverTermT() {
  ...
  // Now copy
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::create(Handle< FermState<T,multi1d<U>,multi1d<U> > > fs,
				   const CloverFermActParams& param_,
				   const QDPExpoCloverTermT<T,U>& from)
  {
...
#pragma omp parallel for
  ...
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::makeClov(const multi1d<U>& f, const RealT& diag_mass)
  {
    START_CODE();
    ...
    dispatch_to_threads(nodeSites, arg, QDPExpoCloverEnv::makeClovSiteLoop<U>);
   ...
   */
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::choles(int cb)
  {
    START_CODE();
   ...
   */
  template<typename T, typename U>
  Double QDPExpoCloverTermT<T,U>::cholesDet(int cb) const
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
  ...
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::ldagdlinv(LatticeREAL& tr_log_diag, int cb)
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
    ...
    QDPExpoCloverEnv::LDagDLInvArgs<U> a = { tr_log_diag, tri, cb };
    int num_site_table = rb[cb].numSiteTable();
    dispatch_to_threads(num_site_table, a, QDPExpoCloverEnv::LDagDLInvSiteLoop<U>);
  ...
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::chlclovms(LatticeREAL& tr_log_diag, int cb)
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
    ...
    tr_log_diag = zero;
    QDPExpoCloverEnv::LDagDLInvArgs<U> a = { tr_log_diag, tri, cb};
    dispatch_to_threads(rb[cb].numSiteTable(), a, QDPExpoCloverEnv::cholesSiteLoop<U>);
   ...
   */
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::applySite(T& chi, const T& psi, 
  ...
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::triacntr(U& B, int mat, int cb) const
  {
#ifndef QDP_IS_QDPJIT
    START_CODE();
    ...
    QDPExpoCloverEnv::TriaCntrArgs<U> a = { B, tri, mat, cb };
    dispatch_to_threads(rb[cb].numSiteTable(), a, 
			QDPExpoCloverEnv::triaCntrSiteLoop<U>);
  ...
  template<typename T, typename U>
  Real
  QDPExpoCloverTermT<T,U>::getCloverCoeff(int mu, int nu) const 
  { 
    START_CODE();
...
#warning "Using unrolled clover term - version 2"
   ...
   */
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::apply(T& chi, const T& psi, 
			    enum PlusMinus isign, int cb) const
  {
    ...
    QDPExpoCloverEnv::ApplyArgs<T> arg = { chi,psi,tri,cb };
    ...
    dispatch_to_threads(num_sites, arg, QDPExpoCloverEnv::applySiteLoop<T>);
  ...
  template<typename T, typename U>
  void QDPExpoCloverTermT<T,U>::packForQUDA(multi1d<QUDAPackedClovSite<typename WordType<T>::Type_t> >& quda_array, int cb) const
    {
      typedef typename WordType<T>::Type_t REALT;
      int num_sites = rb[cb].siteTable().size();

      QDPExpoCloverEnv::QUDAPackArgs<REALT> args = { cb, quda_array,tri };
      dispatch_to_threads(num_sites, args, QDPExpoCloverEnv::qudaPackSiteLoop<REALT>);
  ...
  typedef QDPExpoCloverTermT<LatticeFermion, LatticeColorMatrix> QDPExpoCloverTerm;
  typedef QDPExpoCloverTermT<LatticeFermionF, LatticeColorMatrixF> QDPExpoCloverTermF;
  typedef QDPExpoCloverTermT<LatticeFermionD, LatticeColorMatrixD> QDPExpoCloverTermD;
